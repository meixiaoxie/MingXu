...
这是一个最小化的 TypeScript agent runtime 骨架，核心目标是把模型调用、工具执行、配置加载、插件扩展和会话存储拆成清晰的层。

- 运行环境：Node.js 22+，包管理器：pnpm 10+
- 代码风格：ESM + 严格 TypeScript
- 公共 API 统一从 `src/index.ts` 导出，CLI 走 `src/cli`
- 当前主线：CLI 是组合入口，负责把配置、模型、工具、插件和会话存储拼成可运行的 Agent
...
```bash
pnpm build && node dist/cli/entry.js --help
```
运行构建后的 CLI。

> 目前没有单独的 lint 脚本；仓库里主要靠 TypeScript 编译和 Vitest 维持质量。
...
- `src/core/agent-loop.ts`：主循环。每轮调用模型、执行 tool call、把结果塞回历史消息，并在需要时通过会话存储续跑
- `src/core/types.ts`：核心消息、工具、模型输入输出的中性协议
- `src/core/runtime-defaults.ts`：跨配置和运行时共用的默认值

这里最重要的约定是：**模型、工具、历史消息都使用一套中性的内部协议**，不要把某个模型 SDK 的格式直接渗透到其它层。

### 2) `src/models`

这里是模型适配层。

- `src/models/provider-registry.ts`：按配置创建 provider 的注册表
- `src/models/model-runtime.ts` / `src/models/request-builder.ts`：把 core 的中立输入转成模型请求，再把模型响应转回 core 可用的输出
- `src/models/provider-catalog.ts`：注册内置 provider
- `src/models/anthropic-provider.ts`：Anthropic 的薄适配器，负责把内部协议映射到 Messages API

如果未来加别的模型，优先在 registry 和模型层补适配器，不要把 provider 逻辑塞回 core。

### 3) `src/tools`

这里是 agent 能做什么。

- `src/tools/tool-registry.ts`：工具注册和执行入口
- `src/tools/builtin/echo-tool.ts`：最小的回显工具，适合作为联调和烟雾测试
- `src/tools/builtin/read-file-tool.ts`：受限文件读取工具，默认只在配置的根目录内读文件，并做路径逃逸检查

工具执行是 agent loop 的一部分，但工具本身不应该知道模型细节。CLI 会把内置工具和插件工具合并后再交给 Agent。
...
配置里当前支持的核心字段是：模型、系统提示词、最大迭代次数、会话文件和插件列表。

### 5) `src/plugins`

这里是本地插件加载机制。

- `plugin-loader.ts`：动态导入本地 ESM 插件模块，并把插件注册到同一个工具注册表里
- `plugin.ts`：插件接口定义

插件的作用不是另起一套执行链，而是给同一个工具注册表加工具，这样 agent loop 看到的是一份统一的工具列表。
...
会话存储是可选的；配置了 `sessionFile` 时，CLI 会用 `FileSessionStore` 续写同一段对话历史。

### 7) `src/cli`

这里把所有东西串起来。

流程是：解析参数 → 读取配置 → 创建 provider registry → 包装成 runtime model → 建立工具注册表 → 加载插件 → 选择会话存储 → 构建 `Agent` → 执行 prompt → 输出结果。

也就是说，CLI 不是另一套业务逻辑，而是整个运行时的组合入口。
...
- `tests/file-session-store.test.ts`：文件会话存储
...
3. `src/core/types.ts`：统一消息、工具和模型协议
4. `src/core/agent-loop.ts`：核心执行循环
5. `src/models/provider-registry.ts`：provider 注册与创建
6. `src/models/model-runtime.ts`：模型结果和 core 之间的桥接
7. `src/tools/tool-registry.ts`：工具如何注册和执行
8. `src/memory/file-session-store.ts`：会话持久化实现
9. `src/models/anthropic-provider.ts`：模型适配方式
